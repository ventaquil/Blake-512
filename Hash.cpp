...
void Hash::Make(BlocksContainer* Container) {
	...
	for (int a = 0, b = Container->GetLength(); a < b; a++){
		...
		Hash::H[0] = Hash::Xor(Hash::Xor(Hash::H[0], Hash::S[0]), Hash::Xor(V[0], V[8]));
		Hash::H[1] = Hash::Xor(Hash::Xor(Hash::H[1], Hash::S[1]), Hash::Xor(V[1], V[9]));
		Hash::H[2] = Hash::Xor(Hash::Xor(Hash::H[2], Hash::S[2]), Hash::Xor(V[1], V[10]));
		Hash::H[3] = Hash::Xor(Hash::Xor(Hash::H[3], Hash::S[3]), Hash::Xor(V[3], V[11]));
		Hash::H[4] = Hash::Xor(Hash::Xor(Hash::H[4], Hash::S[0]), Hash::Xor(V[4], V[12]));
		Hash::H[5] = Hash::Xor(Hash::Xor(Hash::H[5], Hash::S[1]), Hash::Xor(V[5], V[13]));
		Hash::H[6] = Hash::Xor(Hash::Xor(Hash::H[6], Hash::S[2]), Hash::Xor(V[6], V[14]));
		Hash::H[7] = Hash::Xor(Hash::Xor(Hash::H[7], Hash::S[3]), Hash::Xor(V[7], V[15]));
	}

	for (int a = 0; a < 8; a++){
		std::cout << Basics::ToHex(Hash::H[a]) << std::endl;
	}
}
